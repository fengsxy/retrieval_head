...
## UV-based Offline Bundle

Use UV to bundle this project and its Python dependencies for transfer to another machine without internet.

Steps on source machine:

- Install UV if missing: see https://docs.astral.sh/uv/
- Create bundle: `bash uv_pack.sh`
  - Writes a pinned `requirements.txt`
  - Downloads wheels into `wheels/`
  - Produces `../Retrieval_Head_uv_bundle.tgz`

Steps on target machine:

- Extract: `tar -xzf Retrieval_Head_uv_bundle.tgz && cd Retrieval_Head`
- Create venv and install:
  - Offline (preferred): `OFFLINE=1 bash uv_install.sh`
  - From index (if online): `bash uv_install.sh`
- Activate: `source .venv/bin/activate`

Notes:

- For GPU/large binary packages (e.g., torch/faiss), ensure the target OS/arch matches the source. Consider pinning a widely supported Python (e.g., 3.10/3.11) and setting `PY_VERSION=3.11` when running `uv_install.sh`.
- Default tar excludes: `.git`, `__pycache__`, `.uv_cache`, `.venv`. Customize via `EXCLUDES` when running `uv_pack.sh`.
