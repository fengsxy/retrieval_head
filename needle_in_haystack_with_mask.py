...
from source.modeling_qwen2_new import Qwen2ForCausalLM
...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if(self.model_provider not in ["OpenAI", "Anthropic"]):
            ...
            if getattr(config, "model_type", "").lower() == "llada":
                ...
                self.model_to_test = Qwen2ForCausalLM.from_pretrained(
                       model_name,torch_dtype="auto",device_map='auto',attn_implementation="eager",trust_remote_code=True,
                    )
        ...
        self.block_list: List[Tuple[int, int]] = []
        self._head_score_ranking: List[Tuple[Tuple[int, int], float]] = []
        self._random_exclusion: Set[Tuple[int, int]] = set()
        if self.mask_topk != 0:
            score_model_name = "Mistral-7B-v0.2-hf" if model_name == "Mistral-7B-Instruct-v0.2" else model_name
            self.score_model_name = score_model_name
            if self.is_llada:
                score_model_name="llada-block-2500"
                self.score_model_name= score_model_name
            if self.is_dream:
                score_model_name="dream"
                self.score_model_name= score_model_name

            score_path = f"head_score/{score_model_name}.json"
            print(score_path)
            if os.path.exists(score_path):
                with open(score_path, "r") as file:
                    raw_scores = json.loads(file.readline())
                scored_heads: List[Tuple[Tuple[int, int], float]] = []
                for key, values in raw_scores.items():
                    try:
                        layer_idx_str, head_idx_str = key.split("-")
                        layer_idx, head_idx = int(layer_idx_str), int(head_idx_str)
                        score = float(np.mean(values))
                        scored_heads.append(((layer_idx, head_idx), score))
                    except (ValueError, TypeError):
                        continue
                scored_heads.sort(key=lambda x: x[1], reverse=True)
                self._head_score_ranking = scored_heads
                top_for_exclusion = min(100, len(scored_heads))
                self._random_exclusion = {entry[0] for entry in scored_heads[:top_for_exclusion]}
            elif self.mask_topk > 0:
                raise FileNotFoundError(
                    f"Head score file '{score_path}' not found. "
                    "Run retrieval_head_detection.py first or place the score file in head_score/."
                )
            if self.mask_topk > 0:
                if not self._head_score_ranking:
                    raise ValueError("No retrieval heads available to mask. Ensure retrieval_head_detection has been run.")
                limit = min(self.mask_topk, len(self._head_score_ranking))
                self.block_list = [self._head_score_ranking[i][0] for i in range(limit)]
                print(self.block_list)
                print(f"masking out top {limit} retrieval heads")
            else:
                if not self._head_score_ranking:
                    print(f"masking out random {-self.mask_topk} heads (no retrieval scores available for exclusions)")
                else:
                    print(f"masking out random {-self.mask_topk} heads (excluding top scored heads)")
    ...
    def retrieval_calculate(
        ...
        if block_mode:
            assert block_row_range is not None, "block_row_range must be provided when block_mode=True"
            ...
            for layer_idx in range(self.layer_num):
                for head_idx in range(self.head_num):
                    ...
                    denom = block_attn.sum(dim=1) + 1e-8
                    needle_ratio = (
                        block_attn[:, self.needle_start:self.needle_end].sum(dim=1) / denom
                    ).mean().item()
        ...
        for layer_idx in range(self.layer_num):
            for head_idx in range(self.head_num):
                row = attention_matrix[layer_idx][0][head_idx][row_idx]
                values, idx = row.topk(topk)
                for v, i in zip(values, idx):
                    if self.needle_start <= i < self.needle_end and inp is not None and inp.item() == self.prompt_ids[i].item():
    ...
    def decode(self, q_outputs, inp, decode_len, block_list=None):
        ...
        for step_i in range(decode_len):
            ...
            if block_list is not None:
                decode_kwargs["block_list"] = block_list
            ...
            self.retrieval_calculate(outputs.attentions, retrieval_score, inp, step_token, row_idx=-1)
    ...
    def construct_random_head(self, n):
        ...
        candidate_heads = [
            ...
            for layer_idx in range(self.layer_num)
            for head_idx in range(self.head_num)
            if (layer_idx, head_idx) not in self._random_exclusion
        ]
        print(len(candidate_heads))
        if n > len(candidate_heads):
            raise ValueError(
                f"Requested {n} random heads but only {len(candidate_heads)} are available after exclusions."
            )
        return random.sample(candidate_heads, n)
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if self.mask_topk > 0:
            ...
            if "block" in getattr(self, "score_model_name", ""):
                save_name = f"llada_block_top_{self.mask_topk}_block_head"
        elif self.mask_topk < 0:
            block_list = self.construct_random_head(-self.mask_topk)
            save_name = f"{self.model_version}_block_random{-self.mask_topk}"
            if "block" in getattr(self, "score_model_name", ""):
                save_name = f"llada_block_top_random_{self.mask_topk}_block_head"
        else:
            if "block" in getattr(self, "score_model_name", ""):
                save_name = "llada_block_top_block_head"
        ...
        with torch.no_grad():
            if self.is_dream:
                output, retrieval_score = self.dream_decode(
                    self.model_to_test,
                    input_ids,
                    steps=32,
                    max_new_tokens=32,
                ...
                response = self.enc.batch_decode(output, skip_special_tokens=True)[0].strip()
                ...
                output, retrieval_score = self.llada_decode(
                    self.model_to_test,
                    input_ids,
                    steps=32,
                    gen_length=32,
