...
try:
    ...
    def dummy_rope_init(config):
        return None
    ROPE_INIT_FUNCTIONS = {"default": dummy_rope_init}
...
if is_flash_attn_2_available():
    try:
        ...
        def _flash_attention_forward(*args, **kwargs):
            raise NotImplementedError("FlashAttention2 is not available in this transformers version.")
else:
    def _flash_attention_forward(*args, **kwargs):
        raise NotImplementedError("FlashAttention2 not detected.")
...
class DreamAttention(nn.Module):
    ...
    def _get_masked_head_indices(
        self,
        block_mask: Optional[Sequence[Tuple[int, int]]],
    ) -> Tuple[int, ...]:
        if not block_mask or self.layer_idx is None:
            return ()
        masked: Set[int] = set()
        for layer_idx, head_idx in block_mask:
            if layer_idx == self.layer_idx:
                try:
                    ...
                    continue
                if 0 <= head_idx_int < self.num_heads:
    ...
    def forward(
        ...
        masked_head_indices = self._get_masked_head_indices(block_mask)
        if masked_head_indices:
            query_states[:, masked_head_indices, :, :] = 0
