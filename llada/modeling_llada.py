...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        masked_heads: Set[int] = set()
        if block_mask:
            for pair in block_mask:
                try:
                    layer_idx, head_idx = pair  # type: ignore[misc]
                except (TypeError, ValueError):
                    continue
                if int(layer_idx) == self.layer_id:
                    masked_heads.add(int(head_idx))
        ...
        if masked_heads:
            ...
            for head_idx in masked_heads:
                if 0 <= head_idx < self.config.n_heads:
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None and not output_attentions:
            ...
            att, cache ,attn_weights = self.attention(
                q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, block_mask=block_mask,output_attentions=output_attentions,
            )
